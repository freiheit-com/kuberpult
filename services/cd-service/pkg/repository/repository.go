...
func (s *State) WriteAllCommitEvents(ctx context.Context, transaction *sql.Tx, dbHandler *db.DBHandler) error {
	...
	if err != nil {
		return fmt.Errorf("could not read commits dir: %v\n", err)
	}
	for _, currentPrefix := range commitPrefixes {
		...
		if err != nil {
			return fmt.Errorf("could not read commit directory '%s': %v", currentpath, err)
		}
		for _, currentSuffix := range commitSuffixes {
			...
			if err != nil {
				return fmt.Errorf("could not read events directory '%s': %v", currentpath, err)
			}
			for i := range potentialEventDirs {
				...
				if oneEventDir.IsDir() {
					...
					if err != nil {
						return fmt.Errorf("could not read event type '%s': %v", fs.Join(currentpath, fileName), err)
					}
