	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			err = r.DB.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
				_, _, _, err2 := repo.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, tc.Transformer)
				if err2 != nil {
					return err2
				}
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			err = r.DB.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
				...
				_, _, _, batchError = r.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, tc.Transformers...)
				if batchError != nil {
					return batchError
				}
				return nil
...
func TestFilterManifestLines(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			t.Parallel()
			ctx := time.WithTimeNow(testutilauth.MakeTestContext(), timeNowOld)
...
func TestExtendAAEnvironment(t *testing.T) {
	type TestCase struct {
		Name                      string
		Transformers              []Transformer
		expectedEnvironmentConfig map[types.EnvName]config.EnvironmentConfig
	...
	testCases := []TestCase{
		{
			Name: "Create empty AA Environment and extend it",
			Transformers: []Transformer{
				&CreateEnvironment{
					Environment: "staging",
					Config: config.EnvironmentConfig{
		...
		{
			Name: "Create empty AA Environment and extend it twice",
			Transformers: []Transformer{
				&CreateEnvironment{
					Environment: "staging",
					Config: config.EnvironmentConfig{
		...
		{
			Name: "Extending an environment that does not exist results in an error",
			Transformers: []Transformer{
				&CreateEnvironment{
					Environment: "development",
					Config:      testutil.MakeEnvConfigLatest(nil),
...
func TestDeleteEnvFromAppWithDB(t *testing.T) {
	const appName types.AppName = "app"
	const envName types.EnvName = "env"
	setupTransformers := []Transformer{
		&CreateEnvironment{
			Environment: "env",
			Config: config.EnvironmentConfig{
			...
			Transforms: []Transformer{
				&CreateApplicationVersion{
					...
					Manifests: map[types.EnvName]string{
						...
						types.EnvName(envName): "testmanifeststaging",
					},
					SourceCommitId: "0000000000000000000000000000000000000000",
					SourceAuthor:   "testmail@example.com",
					SourceMessage:  "test",
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			err := repo.State().DBHandler.WithTransaction(ctxWithTime, false, func(ctx context.Context, transaction *sql.Tx) error {
				// each test should delete the exact same thing:
				_, _, _, err := repo.ApplyTransformersInternal(ctx, transaction, setupTransformers...)
				if err != nil {
					return err
				}
				transformsWithDeleteAtTheEnd := append(tc.Transforms, &DeleteEnvFromApp{
					Application: appName,
					Environment: envName,
				})
				_, _, _, err = repo.ApplyTransformersInternal(ctx, transaction, transformsWithDeleteAtTheEnd...)
				if err != nil {
			...
			if err != nil {
				t.Fatalf("expected no error in 'given' section, got %v", err)
			}
			err = repo.State().DBHandler.WithTransaction(ctxWithTime, false, func(ctx context.Context, transaction *sql.Tx) error {
				releases, err2 := repo.State().DBHandler.DBSelectReleasesByAppLatestEslVersion(ctx, transaction, appName, true)
				if err2 != nil {
				...
				for releaseNumber, expectedManifests := range tc.ExpectedManifestsByEachRelease {
					...
					for env, manifest := range release.Manifests.Manifests {
						if diff := cmp.Diff(manifest, expectedManifests[env]); diff != "" {
							return fmt.Errorf("error mismatch Manifests[%s] - want, +got:\n%s", env, diff)
						}
					}
				}
				...
				if actualSyncStatus != nil {
					return fmt.Errorf("we delete the env from the appp, but the sync status is still there: %v", actualSyncStatus)
				}
				return nil
			})
			if err != nil {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			for _, transformer := range tc.Transformers {
				err := r.State().DBHandler.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
					_, _, _, err2 := r.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, transformer)
					if err2 != nil {
						return err2
					}
					return nil
...
func TestCreateUndeployDBState(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			r := repo.(*repository)
			err := r.State().DBHandler.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
				_, s, _, err := repo.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, tc.Transformers...)
...
func TestTimestampConsistency(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			r := repo.(*repository)
			err := r.State().DBHandler.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
				_, state, _, err2 := r.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, tc.Transformers...)
				if err2 != nil {
					return err2
				}
...
func TestUpdateDatadogEventsInternal(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			ctx := time.WithTimeNow(testutilauth.MakeTestContext(), gotime.Unix(0, 0))
...
func TestChangedApps(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			err := r.State().DBHandler.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
				_, _, _, err := repo.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, tc.Setup...)
				if err != nil {
					return err
				}
				return nil
			...
			err = r.State().DBHandler.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
				_, _, changes, err := repo.ApplyTransformersInternal(testutilauth.MakeTestContext(), transaction, tc.Transformers...)
				if err != nil {
					return err
				}
