...
func TestGetAllApplications(t *testing.T) {
	tcs := []struct {
	...
	}{
		{
			...
			applicationsJson: []byte(`{"apps":["app1"]}`),
			expected:         []string{"app1"},
		},
		{
			name:             "Multiple applications",
			applicationsJson: []byte(`{"apps":["app1","app2","app3"]}`),
			expected:         []string{"app1", "app2", "app3"},
		},
		{
			name:             "No applications",
			applicationsJson: []byte(`{"apps":[]}`),
			expected:         []string{},
		},
	}
	for _, tc := range tcs {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			applications, err := getAllApplications(tc.applicationsJson)
			if err != nil {
				t.Fatalf("Error getting all applications: %v", err)
			}
			if len(applications) != len(tc.expected) {
				t.Fatalf("Expected %d applications, got %d", len(tc.expected), len(applications))
			}
			for i, env := range applications {
				if env != tc.expected[i] {
					t.Fatalf("Expected %s, got %s", tc.expected[i], env)
				}
			}
