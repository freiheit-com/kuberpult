...
func TestDeployedOverviewService(t *testing.T) {
	tcs := []struct {
		Name  string
		Setup []repository.Transformer
		Test  func(t *testing.T, svc *OverviewServiceServer)
	}{
		{
			...
			Test: func(t *testing.T, svc *OverviewServiceServer) {
				resp, err := svc.GetDeployedOverview(context.Background(), &api.GetDeployedOverviewRequest{})
				if err != nil {
					t.Fatal(err)
				}
				if len(resp.Environments) != 3 {
				...
				if len(resp.Applications["test"].Releases) != 1 {
					t.Errorf("expected one important release, got %q", resp.Applications["test"].Releases)
				}
				// Check Dev
				dev, ok := resp.Environments["development"]
				if !ok {
		...
		{
			Name: "A simple overview works with deploys in different environments",
			Setup: []repository.Transformer{
				&repository.CreateEnvironment{
					Environment: "development",
					Config:      config.EnvironmentConfig{},
			...
			Test: func(t *testing.T, svc *OverviewServiceServer) {
				...
				if len(resp.Applications["test"].Releases) != 2 {
					t.Errorf("expected one important release, got %q", resp.Applications["test"].Releases)
				}
				// Check Dev
				dev, ok := resp.Environments["development"]
		...
		},
		{
			...
			Test: func(t *testing.T, svc *OverviewServiceServer) {
				ctx, cancel := context.WithCancel(context.Background())
				ch := make(chan *api.GetDeployedOverviewResponse)
				stream := mockOverviewService_StreamDeployedOverviewServer{
					Results: ch,
					Ctx:     ctx,
				}
				wg := sync.WaitGroup{}
				...
				go func() {
					defer wg.Done()
					err := svc.StreamDeployedOverview(&api.GetDeployedOverviewRequest{}, &stream)
					if err != nil {
						t.Fatal(err)
					}
				}()
				...
				deployedReleases := overview2.Applications["test"].Releases
				if len(deployedReleases) != 1 {
					t.Fatalf("Expected two important releases, got %q", deployedReleases)
				}
		...
		{
			Name: "A stream overview works with deploys in different environments",
			Setup: []repository.Transformer{
				&repository.CreateEnvironment{
					Environment: "development",
					Config:      config.EnvironmentConfig{},
