...
func DBReadCustomMigrationCutoff(h *db.DBHandler, ctx context.Context, transaction *sql.Tx, requestedVersion *api.KuberpultVersion) (_ *api.KuberpultVersion, err error) {
...
LIMIT 1;`)
	...
	if err != nil {
		...
		return nil, fmt.Errorf("migration_cutoff: Error scanning row from DB. Error: %w", err)
	}
	err = rows.Close()
	if err != nil {
		return nil, fmt.Errorf("migration_cutoff: row closing error: %v", err)
	}
	err = rows.Err()
	if err != nil {
		return nil, fmt.Errorf("migration_cutoff: row has error: %v", err)
	}
...
func DBUpsertCustomMigrationCutoff(h *db.DBHandler, ctx context.Context, tx *sql.Tx, kuberpultVersion *api.KuberpultVersion) (err error) {
	span, ctx := tracer.StartSpanFromContext(ctx, "DBUpsertCustomMigrationCutoff")
	defer func() {
		span.Finish(tracer.WithError(err))
	}()
	...
	if err != nil {
		return fmt.Errorf("DBWriteCustomMigrationCutoff: Error reading transaction timestamp from DB. Error: %w", err)
	}
