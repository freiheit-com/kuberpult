...
func (u *ReleaseTrain) Transform(
	...
	for _, currentDeployment := range deployments {
		...
		loopSpan.SetTag("kuberpult_app", currentDeployment.App)
		loopSpan.SetTag("kuberpult_env", currentDeployment.Env)
		...
		if envConfig.Upstream == nil || (envConfig.Upstream.Environment == "" && !envConfig.Upstream.Latest) {
			err = fmt.Errorf("could not find upstream config for env '%s'", currentDeployment.Env)
