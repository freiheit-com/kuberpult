...
func (h *DBHandler) BeginTransaction(ctx context.Context, readonly bool) (*sql.Tx, error) {
	return h.DB.BeginTx(ctx, &sql.TxOptions{
		Isolation: sql.LevelReadCommitted, // Otherwise we would get pq: could not serialise access due to concurrent update error while creating releases in parallel
		ReadOnly:  readonly,
	})
}
