	...
	for _, tc := range tcs {
		t.Run(tc.Name, func(t *testing.T) {
			...
			for index, appBracket := range tc.AddAppBrackets {
				err := dbHandler.WithTransaction(ctx, false, func(ctx context.Context, transaction *sql.Tx) error {
					err := HandleBracketsUpdate(ctx, dbHandler, transaction, appBracket.App, ResolveBracketName(appBracket.App, appBracket.Bracket), appBracket.Time, TransformerID(10+index))
					if err != nil {
						return fmt.Errorf("error while writing release, error: %w", err)
					}
					return nil
