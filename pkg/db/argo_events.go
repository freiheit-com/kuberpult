...
func (h *DBHandler) Dump(ctx context.Context, tx *sql.Tx) error {
	span, ctx, onErr := tracing.StartSpanFromContext(ctx, "DBReadArgoEvent")
	defer span.Finish()
	if h == nil {
		return nil
	...
	if tx == nil {
		return onErr(fmt.Errorf("DBReadArgoEvent: no transaction provided"))
	}

	selectQuery := h.AdaptQuery("SELECT app, env, json, discarded FROM argo_cd_events;")
	row, err := tx.QueryContext(
		ctx,
		selectQuery,
	)
	if err != nil {
		return onErr(fmt.Errorf("error reading argo cd events . Error: %w\n", err))
	}
	defer func(rows *sql.Rows) {
		err := rows.Close()
		if err != nil {
	...
	for row.Next() {
		...
		if err != nil {
			...
			return onErr(fmt.Errorf("Error table for next argo_cd_events. Error: %w\n", err))
		}
		fmt.Println(event)
	}
	err = closeRows(row)
