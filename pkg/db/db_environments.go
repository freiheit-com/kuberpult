...
func processEnvironmentRows(ctx context.Context, rows *sql.Rows) (*[]DBEnvironment, error) {
	defer func(rows *sql.Rows) {
		err := rows.Close()
		if err != nil {
			logging.FromContext(ctx).Error("error while closing row of environments.", zap.Error(err))
		}
	}(rows)
...
func (h *DBHandler) DBSelectAllEnvironments(ctx context.Context, transaction *sql.Tx) (_ []types.EnvName, err error) {
	...
	defer func(rows *sql.Rows) {
		err := rows.Close()
		if err != nil {
			logging.FromContext(ctx).Error("error while closing row on environments table.", zap.Error(err))
		}
	}(rows)
...
func (h *DBHandler) DBSelectEnvironmentApplications(ctx context.Context, transaction *sql.Tx, envName types.EnvName) (_ []types.AppName, err error) {
	...
	defer func(rows *sql.Rows) {
		err := rows.Close()
		if err != nil {
			logging.FromContext(ctx).Error("error while closing row on releases table.", zap.Error(err))
		}
	}(rows)
