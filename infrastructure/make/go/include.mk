...
.PHONY: unit-test
unit-test:
	docker run --user "$(shell id -u):$(shell id -g)" --rm -w $(SERVICE_DIR) --network host -v ".:$(SERVICE_DIR)" -v $(MIGRATION_VOLUME) $(PKG_VOLUME) $(BUILDER_IMAGE) sh -c "cd $(ROOT_DIR)/pkg && oapi-codegen -generate "std-http-server" -o publicapi/server-gen.go -package publicapi publicapi/api.yaml"
