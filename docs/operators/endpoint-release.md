...
* `X-Client-UUID` is an optional header that is used to identify the client that is making the request. It is used to trace requests in datadog and in the logs while debugging.
* `version` (required)
...
### `experimentalArgoBracket`
EXPERIMENTAL: This feature is still under construction.
