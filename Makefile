...
integration-test:
	IMAGE_TAG=$(IMAGE_TAG_KUBERPULT) BUILDER_IMAGE=$(DOCKER_REGISTRY_URI)/infrastructure/docker/builder:pr-$(VERSION) make -C ./pkg gen
...
commitlint: $(COMMIT_MSG_FILE)
	docker run --rm -w /commitlint -v "./commitlint.config.js:/commitlint/commitlint.config.js" -v "./$(COMMIT_MSG_FILE):/commitlint/$(COMMIT_MSG_FILE)" node:18-bookworm sh -c "npm install --save-dev @commitlint/cli@18.4.3 && cat ./$(COMMIT_MSG_FILE) | npx commitlint --config commitlint.config.js"
	rm $(COMMIT_MSG_FILE)
