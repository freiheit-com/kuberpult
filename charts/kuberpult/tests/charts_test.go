...
import (
	...
	"os"
	"os/exec"
	k8sYam "sigs.k8s.io/yaml" //Needed to properly parse the file generated by helm, "gopkg.in/yaml.v3" cannot do it properly
	"strconv"
	"strings"
	"testing"
)
...
func CheckForEnv(target core.EnvVar, strict bool, deployment *apps.Deployment) bool {
...
func getDeployments(fileName string) (map[string]apps.Deployment, error) {
	...
	for _, document := range deploymentYAML {
		if checkDeployment(strings.Split(document, "\n")) {
			...
			if err := k8sYam.Unmarshal([]byte(document), &target); err != nil {
...
func TestHelmChartsKuberpultEnvVariables(t *testing.T) {
	...
	tcs := []struct {
		...
		},
		{
			...
			ExpectedMissing: []core.EnvVar{},
			checkValues:     false, //Don't check the values
		},
		{
			...
			ExpectedMissing: []core.EnvVar{},
			checkValues:     true, //Don't check the values
		},
		{
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			if values, err := readValuesFile(); err != nil {
				...
				if yamlData, err := yaml.Marshal(values); err != nil {
					t.Fatal("Error Marshaling values file.")
				} else {
					tempValuesFile := "vals" + "_" + testId + ".yaml"
					tempValuesFile = dirName + "/" + tempValuesFile

					err := os.WriteFile(tempValuesFile, yamlData, 0644)
					if err != nil {
					...
					if out, err := getDeployments(outputFile); err != nil {
						...
						for _, env := range tc.ExpectedEnvs {
							if !CheckForEnv(env, tc.checkValues, &targetDocument) {
