...
type infixErrMatcher struct {
	infix string
}

func (e infixErrMatcher) Error() string {
	return e.infix
}

func (e infixErrMatcher) Is(err error) bool {
	return strings.Contains(err.Error(), e.infix)
}
...
`,
		...
		{
			Name: "Invalid rendering combination: requires brackets",
			Values: `
git:
  url: "testURL"
manifestRepoExport:
  ...
  rendering:
    ...
    experimentalRenderBrackets: false
    experimentalRootAppsPointToBrackets: true
`,
			ExpectedError: infixErrMatcher{"Cannot point to brackets when not rendering brackets"},
		},
		{
			Name: "Invalid rendering combination: requires apps",
			Values: `
git:
  url: "testURL"
manifestRepoExport:
...
func makeAllIngressPaths(withDex, withUi, withOldApi, withNewApi bool) []networking.HTTPIngressPath {
	...
	if withUi {
		result = append(result,
			...
			makeIngressPrefixPath("/api.v1.ManifestExportGitService/"),
			makeIngressPrefixPath("/api.v1.ProductSummaryService/"),
